...
def run_ga(
        population_size=50,
        max_steps=5,
        max_steps_increment_interval=5,
        level_name="level1"
):

    POPULATION = population_size
    MAX_GENERATIONS = 200000
    MAX_FITNESS = [9992, 9988, 11486, 15981, 20478, 18954, 77443]
    CAPPED_MAX_STEPS = 100
    ...
    population = [[random.choice(['w','a','s','d']) for _ in range(current_max_steps)] for _ in range(POPULATION)]
    generation = 1
    ...
    while generation <= MAX_GENERATIONS:
        ...
        for sequence in population:
            ...
            fitness = (starting_coins - maze.coins_missing) * 1500
        ...
        if champion_fitness > last_best_fitness:
            ...
            if agent_found_goal:
                current_max_steps = champion_steps - 1
        ...
        if generation % MAX_STEPS_INCREMENT_INTERVAL == 0 and not agent_found_goal:
            ...
            for seq in population:
                if seq != champion_sequence:
                    while len(seq) < current_max_steps:
                        seq.append(random.choice(['w','a','s','d']))
                    if len(seq) > current_max_steps:
                        del seq[current_max_steps:]
    ...
    return generational_improvements